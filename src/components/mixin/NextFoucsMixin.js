import { getNextInputIndex, getLastInputIndex } from "@/common.js";
    ...
    nextFocus(event, index) {
      ...
      if (event.key === "ArrowUp") {
        if (nowPos === 0 && index !== 0) {
          ...
          let lastInput = dom[lastInputIndex].getElementsByTagName(
            "textarea"
          )[0];
          lastInput.focus();
        }
      ...
      if (event.key === "ArrowRight") {
        if (
          ...
          index !== this.currentPageBlocks.length - 1
        ) {
          let nextInputIndex = getNextInputIndex(index, this.currentPageBlocks);
          let nextInput = dom[nextInputIndex].getElementsByTagName(
            "textarea"
      ...
      if (event.key === "Delete" || event.key === "Backspace") {
        if (
          ...
          this.$nextTick(() => {
            let dom = document.getElementsByClassName("block");
            let lastInputIndex = getLastInputIndex(
              index,
